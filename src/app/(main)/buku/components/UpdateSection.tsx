...
const UpdateSection = ({BukuID, Gambar, Deskripsi, Judul, Penerbit, Penulis, TahunTerbit, peminjaman, kategoribuku_relasi}:bukuType) => {
    ...
    const [values, setValues] = useState<bukuType>({
        ...
        Gambar,
        TahunTerbit,
        kategoribuku: kategoribuku_relasi,
        peminjaman: []
    });
    ...
    return (
        <>
            <button onClick={() => setModalCreate(true)} className="bg-green-600 py-2 px-4 rounded-md">
                Update
            </button>
            <Modal className={`${!modalCreate && "!hidden"}`}>
            <div className='flex flex-col gap-4 text-primary-color'>
